...
sub usage
{
        print STDERR <<USAGE;
==================================================================================
Description run Pfam enrichment test
Version:2020-03-01

Options
<Required>
	-pfam		<s> :    gene2pfamfile pfam.anno
...
open IN, $pfam;
open OUT, ">$dir/gene2pfam_formated.txt";
while(my $line=<IN>){
        chomp $line;
        my @array=split /\t/, $line;
		my $id=shift(@array);
...
pfam<-read.table(paste("$dir", "gene2pfam_formated.txt", sep="\/"))
...
write.table(res,file=paste("$dir",paste(groups[i],"identify.pfam", sep="."),sep="\/"),row.names=FALSE,quote=F, sep="\t")
}
END
...
foreach my $i (0 .. (@groups)-1){
	...
	my $path = "$dir/gene2pfam_formated.txt";
	open IN, $path;
	...
	open IN, "$dir/$groups[$i].identify.pfam";
	<IN>;
	open OUT, ">$dir/$groups[$i].identify.pfam.xls";
	print OUT "#Term\tDatabase\tID\tInput_number\tBackground_number\tP_Value\tCorrected_P_Value\tInput\tHyperlink\n";
	...
	while(my $q=<IN>){
		...
		$genename{$temp[0]}=~s/,/\|/g;
		my $link = 'https://pfam.xfam.org/family/'.$temp[0];
		print OUT "$hash4{$temp[0]}\tPFAM\t$temp[0]\t$temp[2]\t$temp[3]\t$temp[1]\t$temp[4]\t$genename{$temp[0]}\t$link\n";
	}
	close IN;
	close OUT;
}
