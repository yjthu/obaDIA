...
pheatmap(df,color=colorRampPalette(rev(c("red","white","blue")))(100),cluster_cols=F, scale="row")
dev.off()
...
### To plot correlationplot with 'GGally' packages.

#p <- ggscatmat(df, columns = 1:dim(df)[2] )
...
#ggsave(paste(args[1],".","corScatmat.png",sep=""), plot=p,type="cairo-png", width=size, height=size, dpi=700)
#ggsave(paste(args[1],".","corScatmat.pdf",sep=""), plot=p, width=size, height=size)
