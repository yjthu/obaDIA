...
#    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_swissprot_parser.pl uniprot_sprot.dat.gz $prefix", "$checkpoint_dir/parse_sprot_dat.ok"));
...
#    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_dat_to_Trinotate_sqlite_resourceDB.pl --sqlite $sqlite_db --create", "$checkpoint_dir/init_sqlite_db.ok"));

#    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_dat_to_Trinotate_sqlite_resourceDB.pl --sqlite $sqlite_db --uniprot_index $prefix.UniprotIndex",
#                             "$checkpoint_dir/uniprot_index_loading.ok") );

#    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_dat_to_Trinotate_sqlite_resourceDB.pl --sqlite $sqlite_db --taxonomy_index $prefix.TaxonomyIndex",
 #                            "$checkpoint_dir/taxonomy_index_loading.ok") );
#    

    ##########
    ## EGGNOG
    $pipeliner->add_commands(new Command("wget --no-check-certificate \"$EGGNOG_DAT_URL\"", "$checkpoint_dir/eggnog_download.ok") );
    ...
    # load 
    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_dat_to_Trinotate_sqlite_resourceDB.pl --sqlite $sqlite_db --eggnog NOG.annotations.tsv.gz.bulk_load",
    ...
    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_dat_to_Trinotate_sqlite_resourceDB.pl --sqlite $sqlite_db --go_obo_tab go-basic.obo.tab",
    ...
    $pipeliner->add_commands(new Command("perl $UTILDIR/PFAM_dat_parser.pl Pfam-A.hmm.gz", "$checkpoint_dir/pfam_parsing.ok"));
    
    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_dat_to_Trinotate_sqlite_resourceDB.pl --sqlite $sqlite_db --pfam Pfam-A.hmm.gz.pfam_sqlite_bulk_load",
    ...
    $pipeliner->add_commands(new Command("perl $UTILDIR/EMBL_dat_to_Trinotate_sqlite_resourceDB.pl --sqlite $sqlite_db --pfam2go pfam2go.tab",
